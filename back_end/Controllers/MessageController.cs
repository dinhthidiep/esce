    ...
    public class ChatController : ControllerBase // Đã thay BaseController bằng ControllerBase
    {
        ...
        public async Task<ActionResult> GetUserForChat()
        {
            try
            {
                ...
                var userId = _userContextService.UserId ?? throw new UnauthorizedAccessException("User ID not found.");
                var rs = await _messageService.GetAllUserForChat(userId);
                return Ok(rs);
            }
            catch (Exception)
            {
        ...
        public async Task<ActionResult> GetChattedUser()
        {
            try
            {
                // UserId là string từ Claims
                var userId = _userContextService.UserId ?? throw new UnauthorizedAccessException("User ID not found.");
                var rs = await _messageService.GetChattedUsers(userId);
                return Ok(rs);
            }
            catch (Exception)
            {
        ...
        public async Task<ActionResult> GetHistory(string toUserId)
        {
            try
            {
                ...
                var userAId = _userContextService.UserId ?? throw new UnauthorizedAccessException("User ID not found.");
                var rs = await _messageService.GetChatHistory(userAId, toUserId);
                return Ok(rs);
            }
            catch (Exception)
            {
