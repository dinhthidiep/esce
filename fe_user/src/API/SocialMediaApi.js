  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 500) {
        // Backend is throwing when serializing comments (e.g. object cycle).
        // We are NOT allowed to change backend, so treat 500 as "no comments"
        // and let the UI keep working.
        try {
          console.warn(`Backend returned 500 for /api/Comment/post/${postId}:`, responseText);
        } catch {
      ...
      // For other non-success codes (400, 403, etc.), bubble the error
      const error = new Error(message);
      error.status = response.status;
      error.details = errorDetails;
      throw error;
...
export const getCurrentUser = async () => {
  try {
    ...
    try {
      ...
      if (!response.ok) {
        // Handle 500 errors gracefully - backend has serialization issues with Account navigation properties
        if (response.status === 500) {
          console.warn(`Backend returned 500 for /api/user/${userId} (likely serialization issue), using localStorage data instead`);
          return null;
        }
...
export const createPost = async (postData) => {
  try {
    ...
    const requestBody = {
      ...
      ArticleTitle: title || null, // Backend expects ArticleTitle, send null if empty to use default
      Hashtags: [], // Send as array, not JSON string
      Images: imageUrl ? [imageUrl] : null // Backend expects Images as array, not ImageUrl
    };
...
// Delete reaction API
// For posts: DELETE /api/PostReaction/unlike/{postReactionId}
// For comments: DELETE /api/CommentReaction/unlike/{commentReactionId}
export const deleteReaction = async (targetType, reactionId) => {
  try {
    const token = localStorage.getItem('token');
    if (!token) {
      throw new Error('Authentication required. Please log in again.');
    ...
    let response;
    if (targetType === 'POST') {
      // DELETE /api/PostReaction/unlike/{postReactionId}
      response = await fetch(`${backend_url}/api/PostReaction/unlike/${reactionId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      // DELETE /api/CommentReaction/unlike/{commentReactionId}
      response = await fetch(`${backend_url}/api/CommentReaction/unlike/${reactionId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      throw new Error(`Unsupported target type: ${targetType}. Must be 'POST' or 'COMMENT'.`);
    }
...
// Note: Backend only supports "like" reactions currently
// For posts: POST /api/PostReaction/like/{postId}
// For comments: POST /api/CommentReaction/like (with body)
export const createReaction = async (reactionData) => {
  try {
    ...
    const targetType = reactionData.targetType; // 'POST' or 'COMMENT'
    const targetId = reactionData.targetId;
    const reactionType = reactionData.reactionType || 'like'; // Default to 'like' since backend only supports it

    // Backend only supports "like" reactions currently
    // If a different reaction type is requested, we'll use "like" instead
    if (reactionType !== 'like') {
      console.warn(`Reaction type "${reactionType}" not supported by backend, using "like" instead`);
    }

    let response;
    if (targetType === 'POST') {
      // POST /api/PostReaction/like/{postId}
      response = await fetch(`${backend_url}/api/PostReaction/like/${targetId}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      // POST /api/CommentReaction/like (with body)
      // Backend expects PostCommentId as string
      const commentLikeDto = {
        PostCommentId: targetId.toString()
      };
      response = await fetch(`${backend_url}/api/CommentReaction/like`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        },
        body: JSON.stringify(commentLikeDto),
      });
    } else {
      throw new Error(`Unsupported target type: ${targetType}. Must be 'POST' or 'COMMENT'.`);
    }
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.error('Create reaction failed - Target type:', targetType, 'Target ID:', targetId, 'Reaction type:', reactionType);
...
export const updatePost = async (postId, postData) => {
  try {
    ...
    const requestBody = {
      PostContent: postContent,
      PosterName: posterName,
      ArticleTitle: title || null, // Backend expects ArticleTitle, send null if empty to keep existing or use default
      Hashtags: [], // Send as array, not JSON string
      Images: imageUrl ? [imageUrl] : null // Backend expects Images as array, not ImageUrl
    };
...
export const getAllPostsApproved = async () => {
  try {
    const token = localStorage.getItem('token');
    const headers = {
      'Content-Type': 'application/json'
    ...
    const response = await fetch(`${backend_url}/api/Post/approved`, {
      method: 'GET',
      headers: headers
    });

    if (!response.ok) {
      let message = 'Failed to load approved posts';
      try {
...
export const getAllPostsPending = async () => {
  try {
    const token = localStorage.getItem('token');
    if (!token) {
      throw new Error('Authentication required. Please log in again.');
    ...
    const response = await fetch(`${backend_url}/api/Post/pending`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
    ...
    if (!response.ok) {
      let message = 'Failed to load pending posts';
      try {
        const err = await response.json();
        message = err.message || message;
      } catch {
...
export const approvePost = async (postId) => {
  try {
    ...
    const response = await fetch(`${backend_url}/api/Post/approve`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
    ...
    if (!response.ok) {
      let message = 'Failed to approve post';
      try {
        const err = await response.json();
        message = err.message || message;
      } catch {
...
export const rejectPost = async (postId, comment) => {
  try {
    ...
    const response = await fetch(`${backend_url}/api/Post/reject`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
    ...
    if (!response.ok) {
      let message = 'Failed to reject post';
      try {
        const err = await response.json();
        message = err.message || message;
      } catch {
...
export const getPostDetail = async (postId) => {
  try {
    ...
    const response = await fetch(`${backend_url}/api/Post/detail/${postId}`, {
      method: 'GET',
      headers: headers
    });
    ...
    if (!response.ok) {
      ...
      let message = 'Failed to load post detail';
      try {
        const err = await response.json();
        message = err.message || message;
      } catch {
...
// Review post (Admin only - request edits)
export const reviewPost = async (postId, comment) => {
  try {
    ...
    const response = await fetch(`${backend_url}/api/Post/review`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
    ...
    if (!response.ok) {
      let message = 'Failed to review post';
      try {
        const err = await response.json();
        message = err.message || message;
      } catch {
