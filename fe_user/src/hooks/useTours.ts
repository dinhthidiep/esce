...
export const useTours = () => {
  ...
  const fetchTours = useCallback(async () => {
    try {
      ...
      const baseURL = axiosInstance.defaults.baseURL || 'http://localhost:5002/api'
      ...
      if (err && typeof err === 'object' && 'response' in err) {
        ...
        if (errorCode === 'ERR_NETWORK' || errorCode === 'ECONNREFUSED') {
          setError('Không thể kết nối đến backend server. Vui lòng đảm bảo backend đang chạy tại http://localhost:5002')
        } else {
