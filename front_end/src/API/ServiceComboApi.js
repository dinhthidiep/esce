...
export const createServiceCombo = async (serviceComboData, imageFile = null) => {
  ...
  // Add all service combo fields
  Object.keys(serviceComboData).forEach(key => {
    if (serviceComboData[key] !== null && serviceComboData[key] !== undefined) {
      formData.append(key, serviceComboData[key]);
    }
  });
...
export const updateServiceCombo = async (id, updateData, imageFile = null) => {
  ...
  Object.keys(updateData).forEach(key => {
    if (updateData[key] !== null && updateData[key] !== undefined) {
      ...
      console.log(`Added to FormData: ${key} = ${updateData[key]}`);
    }
  });
  
  // Add image file if provided (new file selected)
  if (imageFile instanceof File) {
    formData.append('imageFile', imageFile);
    console.log('Added image file to FormData');
  }
  // If imageFile is null, we don't append it, so the backend will preserve the existing image
