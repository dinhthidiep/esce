...
const normalizeNews = (payload: any): NewsDto => {
  ...
  if (Array.isArray(imagesRaw)) {
    ...
    if (imagesRaw.includes('|||IMAGE_SEPARATOR|||')) {
      ...
      const semicolonParts = imagesRaw.split(';').map(p => p.trim()).filter(p => p)
...
export const fetchAllNews = async (): Promise<NewsDto[]> => {
  try {
    ...
    const normalizedNews = data.map((news: any) => {
      try {
...
export const createNews = async (dto: CreateNewsDto): Promise<NewsDto> => {
  ...
  try {
    ...
    const endpoint = '/api/news'
    console.log('[NewsApi] Creating news:', { contentLength: dto.content.trim().length, imagesCount: dto.images?.length || 0 })
...
export const updateNews = async (newsId: number, dto: UpdateNewsDto): Promise<NewsDto> => {
  ...
  try {
    ...
    const endpoint = `/api/news/${validNewsId}`
    console.log('[NewsApi] Updating news:', { newsId: validNewsId, hasContent: !!dto.content, hasImages: !!dto.images })
