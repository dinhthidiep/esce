...
import { createServiceCombo } from '../API/CreateServiceComboApi';
...
const CreateServiceCombo = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Note: HostId is set by the backend from JWT token, so we don't send it
      const submitData = {
        Name: formData.name,
      ...
      const result = await createServiceCombo(submitData);
