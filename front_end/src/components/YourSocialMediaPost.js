  ...
  const fetchPosts = async () => {
    try {
      ...
      const transformedPosts = postsData
        .filter(post => post.AuthorId === userId) // Only show posts by current user
        .map(post => ({
          id: post.Id,
  ...
  const handleDeletePost = async (postId) => {
    ...
    try {
      ...
      getAllPosts().then(postsData => {
        ...
        console.error('Error refreshing posts after delete:', err);
      });
    } catch (err) {
  ...
  return (
    <div className="social-media-page">
      ...
      <div className="left-sidebar">
        ...
        <button className="sidebar-button" onClick={() => navigate('/social-media')}>Về mạng xã hội</button>
