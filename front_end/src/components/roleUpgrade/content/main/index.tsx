...
import type { CertificateType, AgencyCertificate, HostCertificate } from '~/api/instances/RoleUpgradeApi'
import {
  approveCertificate,
...
const statusMeta: Record<string, { label: string; color: 'info' | 'warning' | 'success' | 'error'; bg: string }> = {
...
const toAdminRequest = (certificate: AgencyCertificate | HostCertificate, type: CertificateType): AdminRequest => ({
  certificateId: type === 'Agency' ? certificate.agencyId : certificate.certificateId,
  type,
  applicantName: type === 'Agency' ? certificate.userName ?? '' : certificate.hostName ?? '',
  applicantEmail: type === 'Agency' ? certificate.userEmail ?? '' : certificate.hostEmail ?? '',
  phone: certificate.phone,
  businessName: type === 'Agency' ? certificate.companyName : certificate.businessName,
  licenseFile: type === 'Agency' ? certificate.licenseFile : certificate.businessLicenseFile,
...
export default function MainRoleUpgradeContent() {
  ...
  const [rejectDialog, setRejectDialog] = useState<{ open: boolean; request: AdminRequest | null; comment: string }>({
    open: false,
    request: null,
    comment: ''
  })
  ...
  const canModerate = (status?: CertificateStatus) => status === null || status === undefined || status === 'Pending'
  ...
  return (
    <Stack spacing={3}>
                          ...
                          <Stack spacing={0.8}>
                            <Stack direction="row" spacing={1} alignItems="center">
                              <Avatar
                                sx={{
                                  bgcolor:
                                    request.type === 'Agency' ? alpha('#1976d2', 0.2) : alpha('#9c27b0', 0.2),
                                  color: request.type === 'Agency' ? 'primary.main' : 'secondary.main'
                                }}
                              >
                              ...
                              <Chip label={meta.label} color={meta.color} size="small" sx={{ ml: 'auto' }} />
                            </Stack>
                            <Typography sx={{ color: 'text.secondary' }}>
                              Doanh nghiệp: {request.businessName}
                            </Typography>
                            <Typography sx={{ color: 'text.secondary' }}>Email: {request.applicantEmail}</Typography>
                            <Typography sx={{ color: 'text.secondary' }}>Phone: {request.phone || '---'}</Typography>
      ...
      >
        ...
        <DialogActions>
          <Button onClick={() => setRejectDialog({ open: false, request: null, comment: '' })}>Hủy</Button>
      ...
      >
        ...
        <Typography sx={{ fontSize: '1.3rem' }}>
          ...
          - Chỉ Admin mới có quyền phê duyệt/từ chối yêu cầu nâng cấp vai trò. <br />
          - Sau khi phê duyệt, hệ thống tự động cập nhật vai trò tài khoản.
        </Typography>
      </Alert>
    </Stack>
  )
